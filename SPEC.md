...
2. **Plan** — Opus-class coordinator drafts `PLAN.md` / `SPEC.md`. Bare `/magi.plan` reads `magi/BACKLOG.md` Pending entries as candidates.
...
7. **Commit** — `/magi.commit` (sprint mode) prompts user to backfill A-class drift into PLAN/SPEC, promote C-class to `magi/BACKLOG.md`, and optionally sync root-level docs. Standalone mode handles chore/docs/small fixes. Conventional Commits message generated by main agent.
...
| **2: Process / secondary reference** | `magi/` | Consulted when context is needed; low-frequency reads | `PRD.md`, `TECHSTACK.md`, `BACKLOG.md` | `/magi.init` empty scaffolds; accumulate over time |
| **3: Sprint scope** | `magi/<num>-<slug>/` | per-feature; written by `/magi.plan`, frozen at sprint end | `PLAN.md`, `SPEC.md`, `TASKS.md`, `WORKS.md`, `DRIFT.md` | `/magi.plan` creates; sprint ends → frozen |
...
{
  "state": "PLANNING",
  "sprint_dir": "magi/03-profile-page",
...
| **C. Out-of-scope observations** | New concerns / risks / opportunities surfaced during implementation but not in PLAN/SPEC | `/magi.commit` prompts user; on agreement, appends to `magi/BACKLOG.md` `## Pending` |
...
## Pending
- [ ] <description>
  > from `magi/<num>-<slug>/DRIFT.md` (<ISO date>)

## Promoted to sprints
- ~~<description>~~ → `magi/<num>-<slug>/` (<ISO date>)
```
...
```
1. Find latest sprint folder (magi/<num>-<slug>/)
...
2. If `Status: DETECTED`:
   ...
   - **C class**: per-item prompt `(y promote / n ignore)`. On `y`, append to `magi/BACKLOG.md` `## Pending`.
...
| `/magi.init` | `<cwd>/CLAUDE.md`, `README.md`, `SPEC.md`, `magi/PRD.md`, `magi/TECHSTACK.md`, `magi/BACKLOG.md` | **yes (the cwd project)** |
| `/magi.plan` | `<cwd>/magi/<num>-<slug>/PLAN.md` (or SPEC.md / TICKET.md / HOTFIX.md) | **yes** |
| `/magi.tasks` | `<cwd>/magi/<num>-<slug>/TASKS.md` | **yes** |
| `/magi.review-plan` | `<cwd>/magi/<num>-<slug>/MAGI_PLAN_REVIEW.md` | **yes** |
| `/magi.go` | `<cwd>/magi/<num>-<slug>/WORKS.md` + edits to actual source files | **yes** |
| `/magi.review-code` | `<cwd>/magi/<num>-<slug>/{MAGI_CODE_REVIEW,SINGLE_CODE_REVIEW,DRIFT}.md` | **yes** |
...
| `/magi.web.infra.plan` | `magi/<num>-<slug>/INFRA.md` with Terraform `plan.tfplan` (dry-run only), IAM diff matrix, cost estimate via Infracost, rollback plan with STOP-checklist for irreversible changes | `references/domain/web/infra.md` |
| `/magi.web.ci.spec` | `magi/<num>-<slug>/CI.md` with stage breakdown, secrets/permissions audit, deployment strategy, smoke tests; draft workflow file inside the same dir (never written to live `.github/workflows/`) | `references/domain/web/ci-cd.md` |
