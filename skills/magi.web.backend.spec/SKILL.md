...
blocked=$(jq -r '.disallowed_skills["magi.web.backend.spec"] // empty' <<<"$STATE_JSON")
if [[ -n "$blocked" ]]; then
  reason=$(jq -r '.disallowed_skills["magi.web.backend.spec"].reason' <<<"$STATE_JSON")
  suggest=$(jq -r '.disallowed_skills["magi.web.backend.spec"].suggest' <<<"$STATE_JSON")
